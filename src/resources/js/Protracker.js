...
musicUtils.prototype = {
    Constructor: musicUtils,
    sendEventData: function(event, data) {
        var e = document.createEvent("Event");
        e.initEvent(event, true, true);
        e.customData = data;
        window.dispatchEvent(e);
    },
    processMusicLoadedMessage: function() {
        this.sendEventData("musicLoaded", {});
    },
    processNextRowMessage: function(row) {
        this.sendEventData("nextRow", {
            row: row
        });
    },
    processNextPositionMessage: function(position) {
        this.sendEventData("NextPosition", {
            position: position
        });
    },
    processAudioVolumeMessage: function(v1, v2, v3, v4) {
        this.sendEventData("AudioVolume", {
            ch1: v1,
            ch2: v2,
            ch3: v3,
            ch4: v4
    ...
    processInstrumentMessage: function(channels) {
            this.sendEventData("Instrument", {
                ch1: channels[0].sample,
                ch2: channels[1].sample,
                ch3: channels[2].sample,
                ch4: channels[3].sample
            });
    }
};
...
function Protracker()
{
  ...
  for(var t=0;t<16;t++) this.finetunetable[t]=Math.pow(2, (t-8)/12/8);
...
/* NEUROFLIP code */
Protracker.prototype.loadAndPlay = function(strMod) {
...
Protracker.prototype.load = function(url)
{
    ...
    request.onload = function() {
        this.buffer=new Uint8Array(request.response);
        this.parse();
        this.utils.processMusicLoadedMessage();
...
Protracker.prototype.parse = function()
{
  ...
  for(var i=0;i<this.patterns;i++) {
    ...
    for(var j=0;j<patlen;j++) this.pattern[i][j]=this.buffer[1084+i*patlen+j];
    for(var j=0;j<64;j++) for(var c=0;c<this.channels;c++) {
  ...
  for(var i=0;i<this.samples;i++) {
    ...
    for(var j=0;j<this.sample[i].length;j++) {
...
Protracker.prototype.advance=function(mod) {
  ...
  if (mod.tick>=mod.speed) {
    ...
    if (mod.patterndelay) { // delay pattern
      ...
      if (mod.flags&(16+32+64)) {
        ...
        this.utils.processNextRowMessage(mod.row);
        this.utils.processInstrumentMessage(mod.channel);
  ...
  if (mod.row>=64) { 
    this.utils.processNextPositionMessage(mod.position);
    mod.position++;
    this.utils.processNextRowMessage(mod.row);
    mod.row=0;
    mod.flags|=4; 
...
Protracker.prototype.mix = function(ape) {
  ...
  for(var s=0;s<buflen;s++)
  {
    ...
    {
      mod.advance(mod);
      this.module.utils.processAudioVolumeMessage(mod.channel[0].volume,mod.channel[1].volume,mod.channel[2].volume,mod.channel[3].volume);
